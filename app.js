...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function toggleClasses(elements, ...classNames){
    classNames.forEach(name => elements.classList.toggle(name))
  }
  ...
  function setGuessCount(newCount) {
    ...
    document.getElementById(
      'guesses-remaining'
    ).innerText = `Осталось попыток: ${guessCount}.`;
  }
  ...
  function updateGame(e) {
    if (e.target.tagName === 'LI' && !e.target.classList.contains('disabled')) {
      ...
      e.target.innerText = `${guess} --> Совпадающих букв: ${similarityScore}`;
  ...
  function compareWords(word1, word2) {
    if (word1.length !== word2.length) {
      throw 'Слово должна быть такой же длинны';
    }
    let count = 0;
